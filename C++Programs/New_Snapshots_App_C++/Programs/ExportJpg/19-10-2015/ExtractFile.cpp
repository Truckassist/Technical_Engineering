    ...
    if ((dir = opendir (picfiledir().c_str())) != NULL)
    {
        while ((ent = readdir (dir)) != NULL)
        {
            ...
            if (ext == ".index")
            {
                ...
                delete[] get_ini_open;
                char* get_section = convert_tochar_darray(NewFile);
                int completed = iniReader.ReadInteger(get_section, "Completed", 0);
                delete[] get_section;
                if (completed == 0)
                {
...
void ExtractFile::CheckForExtractFile(string fileP)
{
    ...
    if (counter == offVal)
    {
        ...
        char* ini_open_done = convert_tochar_darray(inifilename());
        char* set_section_done = convert_tochar_darray(fileP + ".picfile.index");
        CIniWriter iniWriter(ini_open_done);
        delete[] ini_open_done;
        iniWriter.WriteInteger(set_section_done, "Completed", 1);
        delete[] set_section_done;
    }
    else
    {
        ...
        iniWriter.WriteInteger(set_section, "Completed", 0);
        delete[] set_section;
